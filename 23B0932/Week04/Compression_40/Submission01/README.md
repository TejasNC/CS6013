# Submission — Week 4, Compression 40%, Track 1
...
this target, spending extra bits on the smallest tensors first. For a 40%
target this lands close to a uniform ~6 bits/weight — the least aggressive
of the three Week 4 targets, so quality loss should be the smallest of the
three. `decompress.py` dequantizes back to `torch.bfloat16` for serving —
this is a fake-quant scheme (no packed GPU kernel), matching Track 1's "no
CUDA-speedup required" scope.
...
```bash
python pipeline/run_pipeline.py --submission 23B0932/Week04/Compression_40/Submission01 --limit 20
```

## dtype

`torch.bfloat16` → mixed-precision (~6 bit, per-tensor, bit-packed); dequantized to `torch.bfloat16` on decompress.
