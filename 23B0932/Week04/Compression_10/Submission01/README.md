# Submission — Week 4, Compression 10%, Track 1
...
this target, spending extra bits on the smallest tensors first. For a 10%
target this lands at roughly 1–2 bits/weight on average — aggressive, and
expect real quality loss; see `docs/PROJECT_SOURCE_OF_TRUTH.md` for why a
uniform integer bit-width alone can't hit targets below ~12.5% (8x) without
either fractional-bit mixing (what this does) or structural pruning.
`decompress.py` dequantizes back to `torch.bfloat16` for serving — this is a
fake-quant scheme (no packed GPU kernel), matching Track 1's "no CUDA-speedup
required" scope.
...
```bash
python pipeline/run_pipeline.py --submission 23B0932/Week04/Compression_10/Submission01 --limit 20
```

## dtype

`torch.bfloat16` → mixed-precision (1–8 bit, per-tensor, bit-packed); dequantized to `torch.bfloat16` on decompress.
